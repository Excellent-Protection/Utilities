        ...
        public Contact_ContactVmProfile()
        {
            CreateMap<ContactVm, Contact>()
                ...
                .ForMember(a => a.CityId, opt => opt.MapFrom(s => s.CityId != null ? new EntityReference(Contact.EntityLogicalName, new Guid(s.CityId)) : null))
                 ...
                 //list
                 .ForMember(a => a.Collections, opt => opt.MapFrom(s => s.RelatedEntities.Contains(new Relationship()
                 { SchemaName = CrmRelationsNameMapping.Contact_Collection }) ? s.RelatedEntities[new Relationship()
                 { SchemaName = CrmRelationsNameMapping.Contact_Collection }].Entities.Select(z => z.ToEntity<Collections>()).ToModelListData<CollectionVm>() : null))

        //.ForMember(a => a.LoyaltyPointsSources, opt => opt.MapFrom(s => s.RelatedEntities.Contains(new Relationship()
        //{ SchemaName = CrmRelationsNameMapping.ContactOwner_LoyaltyPointSource }) ? s.RelatedEntities[new Relationship()
        //{ SchemaName = CrmRelationsNameMapping.ContactOwner_LoyaltyPointSource }].Entities.Select(z => z.ToEntity<LoyaltyPointsSource>()).ToEsadModelListData<LoyaltyPointsSourceVm>() : null))

        //.ForMember(a => a.CustomerLevels, opt => opt.MapFrom(s => s.RelatedEntities.Contains(new Relationship()
        //{ SchemaName = CrmRelationsNameMapping.Contact_CustomerLoyaltyLevel }) ? s.RelatedEntities[new Relationship()
        //{ SchemaName = CrmRelationsNameMapping.Contact_CustomerLoyaltyLevel }].Entities.Select(z => z.ToEntity<CustomerLevel>()).ToEsadModelListData<CustomerLevelVm>() : null));
        ;
    }
    }
}
